...
function wheel(args){
	...
	//对象属性修改事件绑定
	args.setObj=function(obj,key,fun,attach){
					...
					['push','pop','shift','unshift','splice','sort','reverse'
						...
						newArrProto[method] = function mutator() {
							...
							for(var i in bind_item.funs){
								bind_item.funs[i](bValue,attach);
							}
							return res;
						}
					});
							...
							for(var k in node.wh){
								if(node.wh[k].name=="wh-for"&&node.forList!=undefined&&node.forList.length>0){
									if(attach.method=="push"){
										...
										isBindFor=true;
										var fnx="n";
										var fkx=node.wh[k].val.split(":")[2]||"key"; 
										var ftx=node.wh[k].val.split(":")[1]||"item";
									...
									if(attach.method=="splice"){
										...
										if(attach.args.length==2){
											...
											//隐藏可显示项
											for(var index_del=0;index_del<attach.args[1];index_del++){
												ccs[attach.args[0]].hide();
												
												var del_binds=[];
												for(var i in args.bind){
													if(args.bind[i].obj==arr&&args.bind[i].key==attach.args[0]){
														del_binds.push(parseInt(i));
													}
												}
												del_binds.reverse();
												for(var i in del_binds){
													args.bind.splice(del_binds[i],1);
												}
